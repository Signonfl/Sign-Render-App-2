...
sign_file = st.sidebar.file_uploader("Upload signage mockup image (with transparency recommended)", type=["png", "jpg", "jpeg"], key="sign")
st.sidebar.markdown("---")

# Real-world background dimensions (in inches)
bg_width_in = st.sidebar.number_input("Background real width (inches)", min_value=1.0, value=240.0, key="bgw")
bg_height_in = st.sidebar.number_input("Background real height (inches)", min_value=1.0, value=120.0, key="bgh")

# Calibration using known object measurement
H_px = st.sidebar.number_input("Measured object pixel height", min_value=1, value=100, key="Hpx")
H_in = st.sidebar.number_input("Measured object real height (inches)", min_value=1.0, value=80.0, key="Hin")
px_per_in = H_px / H_in if H_in else 0

st.sidebar.markdown("---")
# Overlay and styling options
use_mockup = st.sidebar.checkbox("Use mockup image overlay", value=True, key="mockup")
use_text = st.sidebar.checkbox("Use text overlay", value=False, key="text")
use_dragdrop = st.sidebar.checkbox("Enable drag-and-drop placement", value=False, key="dragdrop")
show_measure = st.sidebar.checkbox("Show measurement annotations", value=False, key="measure")

# Mockup dimensions
sign_width_in = st.sidebar.number_input("Sign real width (inches)", min_value=1.0, value=48.0, key="signw")
sign_height_in = st.sidebar.number_input("Sign real height (inches)", min_value=1.0, value=12.0, key="signh")

# Text styling inputs
text_input = st.sidebar.text_input("Enter sign text", key="txtinput")
font_file = st.sidebar.file_uploader("Upload font (.ttf)", type=["ttf"], key="fontfile")
font_size_in = st.sidebar.number_input("Text height (inches)", min_value=1.0, value=12.0, key="fontsize")
text_color = st.sidebar.color_picker("Text Color", "#FFFFFF", key="txtcol")
stroke_color = st.sidebar.color_picker("Text Stroke Color", "#000000", key="strcol")
stroke_width = st.sidebar.slider("Text Stroke Width (px)", 0, 10, 1, key="strwid")
letter_spacing = st.sidebar.number_input("Letter Spacing (px)", min_value=0, value=0, key="letspa")

# Manual offset if not using drag-and-drop
off_x = st.sidebar.number_input("Overlay X offset (% of bg)", min_value=0.0, max_value=100.0, value=50.0, key="offx")
off_y = st.sidebar.number_input("Overlay Y offset (% of bg)", min_value=0.0, max_value=100.0, value=50.0, key="offy")
